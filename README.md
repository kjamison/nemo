...
2. [Inputs](#inputs)
3. [Outputs](#outputs)
4. [Website interface usage](#website-interface-usage)
5. [Code examples for parsing outputs](#code-examples-for-parsing-outputs)
6. [Requirements](#requirements)
7. [Details of tractography database](#details-of-tractography-database)
8. [Parcellations](#parcellations)
...
* To convert outputs to another format, you will need to use Python. See [examples below](#code-examples-for-parsing-outputs).

# Website interface usage
## General options
* **`E-mail address`** 
    * Notification and results will be sent here when processing is complete
* **`MNI Lesion NIfTI file (or .zip)`**
    * Uploaded file must be a single 1mm MNI152 volume in .nii/.nii.gz format, or a .zip file containing up to 10 separate .nii/.nii.gz files.
* **`Weight streamlines by data fit (SIFT2)`**
    * [x] = Apply SIFT2 weights ([`tcksift2`](https://mrtrix.readthedocs.io/en/latest/reference/commands/tcksift2.html), [Smith 2015](https://pubmed.ncbi.nlm.nih.gov/26163802/)) to each streamline while computing ChaCo scores
    * [ ] = Each streamline is equally weighted
* **`Include smoothed mean images`**
    * [x] = Output will include `_smooth6mm_(mean|stdev).nii.gz`, generated by applying 6mm FWHM gaussian smoothing to each of the 420 reference subject pixelwise ChaCo volumes before averaging.
### Experimental options (Use with caution)
* **`Only include subjects with non-zero denominator in local ratio [Experimental]`**
    * [ ] = By default, average for each location will include all subjects. Subjects with no streamlines terminating in that region, parcel, or pair will be assigned a ChaCo=0 in that location.
    * [x] = When computing the ratio for a given location or pair, only include subjects for which the denominator in that location (total number of streamlines ending there) is nonzero. This option will affect all `(chacovol|chacoconn)_(mean|stdev)` outputs for voxelwise, parcel-wise, pairwise, etc. It will not any `_allref` outputs. It will also create an additional `(chacovol|chacoconn)_denomfrac` output, which is the fraction of subjects with a nonzero denominator for that location or pair. This can be applied offline by the user to threshold the chacovol_mean to exclude locations where only a small number of subjects have data. For instance, `Vchaco[Vdenomfrac<0.01]=0` would set chaco values to zero for voxels where >=99% of the 420 reference subjects have no streamlines.
* **`[INTERNAL USERS ONLY] Accumulate total hits along streamline (Much smaller ChaCo scores) [Experimental]`**
    * [ ] = Default usage assigns a value of 1 to streamline that intersects the binary lesion at any point (see "continuous values" option below for non-binary). All other streamlines are assigned a value 0.
    * [x] = Each streamline is assigned a weight depending on how many lesion voxels it intersects.
        * A streamline of length 100 intersecting a single lesion voxel will be assigned a weight of `1/100`, where a streamline that passes through 10 lesion voxels will be assigned a weight of `10/100`
        * When enabled, the ChaCo score for a region or pair of regions will only be 1 if the lesion mask covers the **entire** length of every streamline between the two. Thus, ChaCo values will tend to be much smaller.
* **`[INTERNAL USERS ONLY] Use continuous values for input lesion (do not binarize) [Experimental]`**
    * [ ] = Default usage automatically binarizes the lesion inputs (`V!=0`). Streamlines either intersect the lesion the lesion or do not.
    * [x] = Skip the binarization step for lesion inputs. Inputs are rescaled so that the maximum value = 1, and then assigns a value to each streamline according to the **maximum** voxel value it passes through.
        * Endpoints (voxels or regions or region pairs) are then given a ChaCo score based on (sum of streamline weights that ended there) / (total number of streamlines that ended there), so a ChaCo=0 means no streamlines encountered nonzero lesion voxels, and a ChaCo=1 would mean every streamline that ends there pased through the “strongest” of the lesions.

## Select tractography algorithm
* **`Deterministic (SD_STREAM)`**
    * Tractography database consists of streamlines generated using the deterministic SD_STREAM algorithm in MRtrix3. 
    * Deterministic tractography is generally more sparse, and may include fewer false positive connections, but may also miss certain pathways.
* **`Probabilistic (iFOD2+ACT)`**
    * Tractography database consists of streamlines generated using the probabilistic iFOD2 algorithm, combined with anatomical constraints (ACT).
    * Probabilistic tractography is generally more densely interconnected, which may capture more pathways, but may also contain false positive connections.
* **`Both Probabilistic and Deterministic`**
    * Compute both SD_STREAM and iFOD2 outputs separately

## Add Resolutions and Parcellations 
* **`Add output resolution`**
    * Add one or more `1mm`, `2mm`, ... or `HCP-2mm-cifti91k`
    * Downsample original 1mm output to the given resolution. Output will include `*_resXmm_*.nii.gz` NIfTI volumes.
    * `HCP-2mm-cifti91k` is a combined cortical surface + subcortical voxel mapping. Output will include `*_cifti91k_*.dscalar.nii`
* **`Add output parcellation`**
    * Add atlas from list (See [Parcellations](#parcellations)) or select `[Upload custom atlas]` to upload your own atlas volume
    * By default, output will include both `chacovol` (region-wise disconnectivity) and `chacoconn` (pairwise disconnectivity)
* Options for each resolution or parcellation include
    * **`Count streamlines that start and end in the same ROI in region-wise output`**
        * [ ] Typically, we ignore streamlines that start and end in the same voxel or region. 
        * [x] For some very coarse parcellations (eg: the Yeo 7- or 17-network parcels), we may want to consider these intra-parcel connections.
    * **`Compute pairwise disconnectivity`**
        * [x] = Output will include `chacoconn` (voxel x voxel) or (region x region) disconnectivity maps.
        * Pairwise disconnectivity is not recommended for high resolutions (1-2mm), as the output files are prohibitvely large to compute and store.
    * **`Output ChaCo for each reference subject (large file size)`**
        * [x] = Output will include separate `chacovol` and `chacoconn` for each of the 420 HCP reference subjects
        * For a single very extensive lesion mask, with 1mm resolution, <code>chacovol\_allref</code> can be as large as 700MB, and <code>chacoconn\_allref</code> can be 10s of GB
* **NOTE:** Streamline/lesion intersections and endpoints are all computed in 1mm space, and those are then downsampled or parcellated to the desired size.
